...
- `contract TARGET [--wallet NAME] [-y] [--simulate] [--json] [--args-json OBJECT] [--scval-xdr NAME BASE64]... [FUNCTION [--NAME VALUE]...]`
- `contract list [--json] | contract add NAME C... [--json] | contract remove NAME [--json]`
- legacy: `contract invoke C... [--wallet NAME] [-y] [--json] [--args-json OBJECT] [--scval-xdr NAME BASE64]... -- FUNCTION [--NAME VALUE]...`
...
The shared `fresnica-client` resolves Stellar Asset Contract, Wasm, and external-reference specs through Stellar RPC. ABI encoding and normalized JSON conversion remain delegated to the official `soroban-spec-tools` implementation, so Terminal does not maintain a parallel Soroban codec. Terminal owns only command grammar, human review/confirmation, and its machine JSON schema; it does not parse `ScSpecEntry` or construct `ScVal`. Agents may pass a typed top-level argument object with the host-side `--args-json OBJECT` option before the function name; Terminal only splits that object by argument name. `fresnica-client` performs narrow shape/type guards around the official parser because the pinned upstream implementation still has panic/permissive edges for malformed UDT JSON and incomplete direct JSON forms for `Void`, `Timepoint`, `Duration`, `Result`, and `Error`. Fresnica adapts `null`/numeric JSON for `Void` and the two time types before official encoding, including nested values and map keys; structs, unions, enums, maps, tuples, options, bytesN, wide integers, and dynamic `Val` inputs remain Contract-Spec checked. Dynamic `Val` JSON uses the explicit serde ScVal form (for example `{"u32":7}`), while typed JSON `Result`/`Error` values fail closed rather than inventing a Fresnica-only representation. External protocols that already return a typed ScVal may instead supply it as base64 XDR with `--scval-xdr NAME BASE64`; Fresnica validates its full recursive shape against the current Contract Spec before normalized review/simulation. Machine review JSON includes both the normalized semantic value and exact `scval_xdr`; read/simulation output likewise includes `result_xdr`, so dynamic values cannot lose their ScVal discriminant during agent handoff. Dynamic help exposes official type examples where available.

The versioned `abi.functions[].inputs[]` model also exposes `composition: { mode, guided }`. This is an additive `fresnica-soroban-abi-v1` field; the legacy flat `functions` JSON is unchanged. `typed_json` means the complete input domain can be safely composed through `--args-json` and therefore `guided=true`; this includes recursively supported Option/Vec/Map/Tuple/struct/union/enum shapes. `dynamic_scval_json` means the type contains Soroban `Val` and requires explicit tagged ScVal JSON, so a finite ABI-driven form must not pretend it can guide the value. `scval_xdr_success_only` is the conservative Result boundary: typed JSON has no stable official representation, while an exact XDR success value can be validated and normalized. `unsupported` means Fresnica has no safe full-domain composition path. The mode describes the safest complete-domain input contract, not every lower-level fallback accepted by the CLI. From the v0.6 freeze checkpoint onward, these mode labels and meanings are stable within `fresnica-soroban-abi-v1`; incompatible semantic changes require a new schema version, and consumers must treat unknown future modes as unguided/unsupported.
