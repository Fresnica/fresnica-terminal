...
def replace(path: str, label: str, old: str, new: str) -> None:
    file = Path(path)
    text = file.read_text()
    count = text.count(old)
    if count != 1:
        raise SystemExit(f"{path}: {label}: expected one refinement target, found {count}")
    file.write_text(text.replace(old, new))


replace(
    "crates/tui/src/app.rs",
    "filter trustline native asset",
...
replace(
    "crates/tui/src/app.rs",
    "open from cache without network refresh",
...
replace(
    ...
    "test trustline native filtering",
