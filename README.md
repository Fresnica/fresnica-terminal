...
Terminal v0.4.0 pins `6c3388c84993bf8dfd133638a7c32a37a0769e74`, the exact Fresnica Rust capability source with versioned Argon2id wallet protection and the System Authentication boundary. Active v0.5 development pins `a19024481fc30b884ed68372749b6e7875e947c3`, adding typed Contract Address names, executable/Wasm observations, raw Wasm metadata, versioned SEP-41 capability evidence, wallet-native token resolution, positional Contract Spec invocation, and headless SEP-41 balance/transfer preparation with exact amount semantics and executable-identity rechecks across multi-RPC token operations. Native SDK v0.3.0 remains the binary SDK baseline (Native Binding API 3 / Universal SDK API 5 / Core Client API 5); Terminal consumes `fresnica-client` / `fresnica-sdk` directly without changing that published Native/UniFFI ABI.
