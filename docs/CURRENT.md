...
- Active Device Authentication branch: `feat/terminal-device-auth-cross-platform`; portable Device Unlock base `feat/terminal-device-unlock@ad8b2fcb0b76a4a62456317ad523343e6c9d4350`; current product checkpoint `003e56aeb6adaa9fcd553f11247cc00cda798ed9`; exact upstream pin `ecdc6bf77741949e4f7043153143824ae78a1b44`. Repository HEAD is authoritative for docs-only follow-up state.
...
- Windows uses Windows Hello as `DeviceAuthenticator` and Credential Manager only as `DeviceSecretStore`. Each interactive transaction calls current-user verification through `IUserConsentVerifierInterop::RequestVerificationForWindowAsync`; only `Verified` permits Credential Manager read. Cancel aborts; unavailable/not-configured/unsupported Hello explicitly requires the fresh Fresnica Passphrase. No UAC, service, LocalSystem broker or helper exists. Current desktop interop requires Windows 11 build 22000+. Physical testing on two Windows machines proved Credential Manager enrollment but exposed a desktop-path bug: the CLI gated the Win32 interop request with `UserConsentVerifier::CheckAvailabilityAsync` and fell back before showing Hello. Product `e4a7449...` removes that transaction preflight and follows the Microsoft desktop flow directly: active HWND -> `IUserConsentVerifierInterop::RequestVerificationForWindowAsync` -> result mapping. A subsequent physical retest on both machines still returned a non-Verified Hello result without UI. Product `003e56a...` now prints the exact non-Verified reason at transaction time and performs `CheckAvailabilityAsync` only after enrollment as a non-blocking advisory warning. Enrollment remains stored even when Hello is currently unavailable, so configuring Hello later does not require re-enrollment. Retest remains open.
- Linux stores the key in a dedicated Secret Service collection aliased `fresnica-device-unlock`, not the user's default/login collection. Each transaction first requires a no-prompt immediate lock, then performs a raw Secret Service `Unlock` and accepts authentication only when the service returns a real `org.freedesktop.Secret.Prompt` whose `Completed` signal is not dismissed. A silent immediate unlock is rejected, relocked and explicitly falls back to the fresh Fresnica Passphrase. End-of-transaction relock is also required to be silent so Fresnica never manufactures a second authentication prompt.
...
- Local deterministic gate for the cross-platform baseline plus Windows fix `e4a7449...`: repository boundary PASS; rustfmt + `git diff --check` PASS; workspace tests PASS (7 Anchor + 49 CLI + 2 CLI contract + 3 presentation + 21 TUI); workspace Clippy `-D warnings` PASS; Linux native release PASS; Windows x64 MSVC all-target check, Clippy `-D warnings` and release PASS. macOS behavior was not changed after its physical acceptance, but the structural Authenticator/SecretStore refactor still requires the normal real macOS release runner before cross-platform integration.
- Physical acceptance remains OPEN for the corrected Windows Hello desktop request and Linux dedicated-collection prompt behavior. Tester guides are `docs/testing/windows-device-unlock-acceptance.md` and `docs/testing/linux-device-unlock-acceptance.md`.
