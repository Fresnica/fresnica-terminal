...
- State is explicit: `unavailable`, `disabled`, `locked`, `ready`. macOS/Linux can report a locked vault and request the OS-native unlock prompt only when signing actually needs the key. If the vault is already unlocked, Fresnica reports `ready` and uses it directly rather than forcing a system-wide relock/re-authentication. Windows Credential Manager has no equivalent vault-lock state in this baseline, so an existing credential is `ready`.
