...
Status: **v0.6.0 = Soroban ABI Composer + documentation. The Composer and documentation gates are complete; repository and tag state remain authoritative for publication.**

Last verified: 2026-09-22.

## Source of truth

- v0.5.0 release baseline: `e455941ea27e2b5f4c3d827a348b4a82f43f485b`, tagged `v0.5.0`. The v0.6.0 Main/tag commit is created by PR #40 squash integration and the release workflow; repository state is authoritative after publication.
- v0.6.0 release branch: `release/v0.6.0`, stacked on documentation-closeout head `c0f37c6c1b74a612e7886b3dea4b4029ece35ddf`; it pins integrated Fresnica shared source `c222be6010e82288cf6a255ece48c5bfd99662dd` (PR #187). Earlier v0.6 checkpoints are shared `ca2893b` / Terminal `0126e85` for the semantic ABI schema, shared `da21d835` / Terminal `c6b4baf` for typed JSON composition, shared `e68c55c` / Terminal `aa18875` for UDT boundary hardening, shared `ad759b0` / Terminal `f76041e` for corpus/value-identity hardening, and shared `188fbbc` / Terminal `7030ace` for the machine-readable composition contract.
...
Typed composition uses `ContractInvokeRequest::add_json_argument(name, serde_json::Value)` and Terminal's host-side `--args-json OBJECT`. Terminal only splits the top-level object; actual ScVal encoding remains delegated to `soroban-spec-tools`, then normalized back through the same Contract Spec before simulation/review. No second ScVal codec exists in Fresnica.

Boundary/corpus testing found concrete gaps in pinned `soroban-spec-tools 28.0.0` (and the same code paths remain present in the checked Stellar CLI source): malformed UDT JSON can reach `todo!()` paths, named structs can ignore extra fields, direct `from_json` does not provide the same `Void`/`Timepoint`/`Duration` conveniences as `from_string`, and `Result`/`Error` have no stable direct composed JSON form. Fresnica therefore keeps a narrow compatibility/safety layer around the official codec: exact struct/tuple/union/enum shape checks, recursive adaptation of `null` and numeric time values including map keys, and full recursive ScVal-vs-Contract-Spec validation before semantic normalization. Typed JSON `Result`/`Error` values fail closed instead of inventing a Fresnica representation; pre-encoded ScVal XDR remains the expert path where a valid value can be normalized safely.

The local real-interface corpus currently contains 9 deployed Testnet contracts and 160 functions. It includes 4 `Val` input occurrences and 57 `Result` output occurrences but no sampled `Result`, `Timepoint`, `Duration`, or `Void` inputs, so those input edges are covered with explicit synthetic Contract-Spec regression vectors rather than false live claims. Supported static typed-JSON values now round-trip `JSON -> ScVal -> normalized JSON -> ScVal` identically. Dynamic `Val` is deliberately different: normalized JSON alone can erase the ScVal discriminant (`u32(7)` and `i32(7)` both render as `7`), so machine review now also carries exact `scval_xdr`; read/simulation output carries `result_xdr` for the same reason.

The final Composer freeze gate on shared `188fbbc` is 257/257 tests PASS. Terminal validation with that exact pin is rustfmt PASS, workspace tests PASS (Anchor 7, CLI 98, CLI integration 5, presentation 3, TUI 21), and workspace Clippy `-D warnings` PASS. Earlier corpus validation on shared `ad759b0` was 254/254 before the composition-support contract was added. Existing real proofs include Blend `Vec<Request>`, smart-account Union/Enum and `Map<UDT,...>`, plus a live smart-account `execute(... target_args: Vec<Val>)` simulation where `{"u32":0}` reached the contract as `[0]` before failing only on Soroban's contract re-entry rule. Testnet XLM SAC `balance` and `decimals` confirm machine responses expose exact argument `scval_xdr` and `result_xdr` alongside normalized values.
...
### v0.6 documentation closeout

v0.6 also treats documentation as product surface, not release notes. `AGENTS.md` is the AI-agent entry point and teaches capability discovery, layer ownership, Soroban composition, plugin selection, safety invariants, and local validation. `docs/soroban-composer.md` is the focused Composer usage contract. `docs/creating-plugins.md` gives humans a from-zero external-plugin path; `docs/creating-plugins-for-agents.md` gives agents the same workflow as a deterministic decision/acceptance path; `examples/plugins/fresnica-xlm-balance` is the canonical minimal executable example. The documentation rule is the same as the architecture rule: discover and reuse public machine capabilities before writing adapters, and keep wallet authority inside Fresnica.

Documentation acceptance was executed rather than inferred. All relative links in the new entry-point/tutorial set resolve. The canonical `fresnica-xlm-balance` example was installed only through PATH, discovered by `fresnica plugin ls --json`, returned exit 2 on invalid input, and completed a live Testnet `token XLM balance ... --json` call through the injected `FRESNICA_PLUGIN_HOST`. The Composer guide's runtime path was separately exercised against the Testnet native XLM SAC: live inspection reported `fresnica-soroban-abi-v1` and `balance(id: Address)` as `typed_json` / `guided=true`; the documented `--args-json` + `--simulate --json` path returned normalized argument value plus exact `scval_xdr`, `result_xdr`, and zero write/auth/event effects. Final Terminal documentation-closeout gate: repository boundary PASS, rustfmt PASS, workspace tests PASS (Anchor 7, CLI 98, CLI integration 5, presentation 3, TUI 21), workspace Clippy `-D warnings` PASS, and the example POSIX shell script passes `sh -n`. No product Rust source or shared-source revision changed in this docs slice.
...
### v0.5 closeout — MultiSigTools Intent-first interoperability (2026-09-15)

The old transaction-first Soroban Preparation/freeze assumptions are retired for MST interoperability. MultiSigTools `origin/main@71f0cfc` now coordinates a durable semantic Intent, recording simulation, immutable detached AuthorizationPlan, append-only AUTH contributions, late-bound execution source, enforcing simulation, and only then a final unsigned transaction. Fresnica does not need an MST-specific adapter and must not expose a generic signing RPC.

Fresh Testnet compatibility evidence used a read-only archive of that MST head. MST's own baseline passed 582/582 tests and its TypeScript/Vite production build. A disposable Fresnica protected signer contributed the exact detached G-account AUTH entry exposed by the Intent AuthorizationPlan; MST independently verified it and reached `authorization_ready`. A different funded executor was then selected only after AUTH completion; enforcing simulation produced the final unsigned transaction while preserving the finalized AUTH entry byte-for-byte. The AUTH signer was not the execution source and therefore did not need to return for a transaction-envelope signature. If the same account is deliberately chosen as both AUTH signer and final transaction source, the later envelope signature remains a Stellar protocol requirement.

`PrivateNote` and `externalReference` are now durable `StoredSorobanIntent.privateContext` and remained available through AUTH and late execution. They are decision context for Human/Agent review, not chain facts and not part of `intentDigest`. Authorization evidence is instead bound to the semantic Intent and versioned `AuthorizationPlan` (`authorizationPlanDigest` + revision); expired plans can be replanned without silently reusing old AUTH. This removes the need for the previously considered long-lived continuation session/grant model.

Earlier Classic MST compatibility also remains valid: unsigned Request -> Fresnica exact-envelope contribution -> ready; pre-signed Sign-Agent Request creation; and partial multisig accumulation preserving prior signatures and transaction hash all passed. The final Fresnica signing primitive is therefore already proven against external Classic envelopes. A final Soroban executor-envelope signing attempt in the remote test environment was blocked before execution by the platform safety layer; no protocol-format incompatibility was observed, and no product claim is made for that blocked hop.

Closeout disposition: v0.5 has no known implementation blocker from MST integration. The reusable boundary remains `inspect -> semantic review -> authorize/contribute`; MST is an external coordinator/consumer, not a special Fresnica signing surface. Further MST workflow features belong to later milestones unless new evidence exposes a Foundation defect.

Closeout local gate on 2026-09-15: Terminal `cargo fmt --all --check`, `git diff --check`, workspace tests (7 Anchor + 92 CLI + 5 CLI integration + 3 presentation + 21 TUI), workspace Clippy with `-D warnings`, and workspace release build all PASS. Shared `fresnica-client@afcb634` formatting and 235/235 tests PASS. A separate non-required `cargo clippy --all-targets -- -D warnings` audit of the upstream rust-client reports pre-existing style lints (for example `too_many_arguments`, `result_unit_err`, `clone_on_copy`); the repository Required CI does not use Clippy, and v0.5 closeout does not broaden into unrelated lint cleanup.

Plugins remain ordinary lower-trust orchestration processes. They should become cheaper as the Operation Foundation improves; they must not become alternate wallet implementations. The generic process boundary is now proven to be the public machine CLI itself: an ephemeral Testnet `fresnica-xlm-balance` shell plugin successfully called `$FRESNICA_PLUGIN_HOST --network "$FRESNICA_PLUGIN_NETWORK" token XLM balance OWNER --json` and received the same native-SAC resolution, SEP-41 evidence and exact balance. Do not duplicate ordinary operations as private `__plugin-host` RPCs; reserve host re-entry for exceptional state/authority boundaries such as SEP-10 bearer-token return or mandatory host-owned proposal review.
...
Machine discovery remains at the 33-operation surface established by `82b4dc0d8278f361c01c46311faa636b52c5afba`: `fresnica capabilities --json` returns versioned schema `fresnica-capabilities-v1` with CLI/source identity plus 33 currently machine-ready operations and their usage/effect/dependencies. Since the first 17-operation checkpoint, Classic payment, trustline mutation and DEX offer writes gained deliberate `-y --json` execution surfaces that reuse the existing prepare/review/authorization/sign/submit paths; local identity discovery now exposes only public wallet/contact metadata through `wallet list --json`, `info --json`, contact list/add/remove JSON, and `wallet use NAME --json`; `plugin ls --json` exposes installed Fresnica plugin names and invocable commands without leaking executable paths; `wallet device-unlock status NAME --json` exposes public unlock readiness without prompting for authentication or performing stale-enrollment migration. The capability command itself remains deliberately zero-state: integration coverage removes HOME/USERPROFILE/FRESNICA_HOME, supplies invalid Horizon/RPC URLs, keeps a normal leading `--network` global option, and still succeeds without WalletStorage or provider initialization. Commands without a deliberate JSON contract are not advertised yet.

Token operation semantics remain headless-owned from upstream `a19024481fc30b884ed68372749b6e7875e947c3`; the active shared-source pin is now `7b7051490abe72758902970e07cf4cf80dd02472`. XLM/native and CODE:GISSUER deterministically resolve to their network-scoped SAC Contract ID while C... remains a direct contract reference; `TokenBalanceRequest` and `TokenTransferRequest` then own SEP-41 function selection, exact decimal/raw conversion, selected-wallet `from` binding, current-interface validation, and executable/Wasm consistency when a custom token operation spans multiple RPC observations. Terminal keeps only saved-name/local-name adaptation, Contract Store provenance, prompting and human/machine presentation. `token TOKEN [--json]`, `token TOKEN balance OWNER [--json]`, and `token TOKEN transfer AMOUNT TO [--wallet NAME] [-y] [--json]` therefore remain the same product surface while native or other Rust consumers can call the same semantic operations without reproducing CLI logic. A post-refactor live Testnet XLM balance remained raw `455350000001`, decimals `7`, amount `45535.0000001`; the earlier watch-only transfer acceptance had already reached real simulation/preparation and failed closed at ledger authorization with no submission output.

Aqua is the first DeFi consumer proof of this boundary. A temporary Testnet `fresnica-aqua` plugin used only Python stdlib, Aquarius HTTP APIs, and Fresnica public machine operations: Router inspection discovered 75 functions directly from deployed Contract Spec (`version=220`, 164 token sets), and real Find Path calls returned working 3-hop/4-hop quotes without Stellar SDK, secret handling, ABI parsing, transaction construction or signing code in the plugin. Aquarius returns `swap_chain_xdr` as an already typed ScVal whose per-hop tuple includes a 32-byte pool index/hash not recoverable from only the API `pools[]`/`tokens[]`; that real route became the upstream regression vector for `7b70514`. Terminal exposes the bounded host-side encoding option as `--scval-xdr NAME BASE64` before the function name so function-owned flags remain untouched. A final live `swap_chained` write-simulation probe was not executed because the remote execution safety layer blocked the command; no claim is made that the write preparation itself was live-validated.

CrossMesh is the next external-consumer proof and required no further Operation Foundation change. A temporary Testnet `fresnica-crossmesh` plugin used Python stdlib plus only Fresnica public machine operations. Through the real plugin dispatcher, `info` dynamically read Circle CCTP Testnet deployments and verified local domain `27`, message/body version `1`, and consistent `MessageTransmitter` / `TokenMessengerMinter` / `CctpForwarder` links; `outbound-plan` resolved a Fresnica wallet name and Testnet USDC to its exact SAC, validated the deployed `deposit_for_burn` signature, and encoded a 20-byte EVM recipient to CCTP `bytes[32]`; `inbound-plan` validated the deployed `mint_and_forward(message: bytes, attestation: bytes)` signature. Bad EVM addresses and non-Testnet use fail closed. A dummy inbound live simulation reached the deployed Forwarder with `Bytes(00), Bytes(11)` and was rejected by the contract with error `#7303`, proving the generic bytes/RPC path. The corresponding outbound `deposit_for_burn` live simulation was blocked by the remote execution safety layer, so no claim is made for that final write-preparation hop. The proof plugin contains no Stellar/Soroban SDK, XDR builder, secret, mnemonic, or private-key handling.

Blend is the third external-consumer proof and likewise required no Operation Foundation change. The official Blend v2 Testnet deployment (`TestnetV2` pool `CCEBVD...4HGF`, pool factory `CDV6RX...NTF6`, backstop `CBDVWX...CLZA`) was inspected live through Fresnica; the deployed Pool exposes 27 functions and its generic `submit(from, spender, to, requests: vec<Request>) -> Positions` surface uses the same Contract-Spec JSON machinery as ordinary contracts. Live reads decoded `PoolConfig`, all four `Reserve { config, data }` values, `Positions`, oracle `Asset` enums / optional `PriceData`, `PoolBackstopData`, `UserBalance`, reward-zone membership and the backstop token without Blend/Stellar SDK code. Official `testnet.contracts.json` Wasm hashes for the lending pool, factory and backstop matched the live executable hashes byte-for-byte. A temporary stdlib-only `fresnica-blend` plugin used the real plugin dispatcher for `info`, `position`, semantic request planning and Backstop reads; action mapping remains protocol-local (`Supply=0`, `Withdraw=1`, `SupplyCollateral=2`, `WithdrawCollateral=3`, `Borrow=4`, `Repay=5`). A live 1-XLM supply simulation passed Blend business validation and reached Fresnica's `prepared_write_no_submit` boundary because the plugin intentionally omitted `-y`; no authorization/signing/submission occurred. A 1-stroop probe was rejected earlier by Blend itself with `#1216 InvalidBTokenMintAmount`, confirming protocol errors remain distinguishable from ABI/Foundation failures. The factory's apparently read-only `is_pool` call is intentionally not used by the plugin because its implementation calls `extend_instance`, and Fresnica correctly classifies the simulation as a write requiring `-y`; deployment verification instead uses the official Wasm hashes without mutating chain state. Unknown actions/assets and non-Testnet use fail closed, and the proof plugin contains no Stellar/Soroban SDK, XDR builder, secret, mnemonic or private-key handling.

The Contract Store commands are local operations and remain usable even when Horizon/RPC configuration is invalid. Machine `contract list --json` uses `fresnica-contract-list-v1`, separate from the on-disk Store schema. Final local validation through the typed ScVal-XDR contract surface: rustfmt and `git diff --check` PASS; workspace Clippy `-D warnings` PASS; workspace tests PASS (Anchor 7, CLI 87, CLI integration 5, presentation 3, TUI 21); workspace release build PASS; repository boundary PASS at upstream `7b705149...`. Upstream rust-client validation is 223/223 tests PASS. The two former CLI numeric-format tests moved with their implementation into the shared client suite rather than being dropped. A fresh-home live Testnet probe of native XLM SAC `CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC` returned and persisted `SEP-41 0.5.1: native_sac=true, sep47_declared=false, interface_compatible=true`.
