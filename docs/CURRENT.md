...
- Active Device Authentication branch: `feat/terminal-device-auth-cross-platform`; portable Device Unlock base `feat/terminal-device-unlock@ad8b2fcb0b76a4a62456317ad523343e6c9d4350`; current product checkpoint `acc979c7e51747f28644f0a8213bbe416097a3b0`; exact upstream pin `ecdc6bf77741949e4f7043153143824ae78a1b44`. Repository HEAD is authoritative for docs-only follow-up state.
...
- The Fresnica Passphrase remains the protection/recovery root. Device Authentication is convenience-only for routine signing; enable/disable require a fresh Passphrase, and Reveal/Export/protection changes remain fresh-Passphrase-only.
- The exact-envelope contract remains unchanged: signer public key + SHA-256(canonical protected envelope) identifies one 32-byte `WalletUnlockKey`. Re-protection invalidates prior enrollment without changing Core cryptography.
- Terminal exposes `wallet device-unlock enable|disable|status NAME`. Non-TTY CLI never activates Device Authentication and there is no CLI session/cache. `-y` may skip Fresnica text confirmation but never platform authentication.
- The platform boundary is now explicitly split into `DeviceAuthenticator` and `DeviceSecretStore`. Authentication proves fresh user presence for the transaction; secret storage only retains the exact-envelope unlock key. Platform glue composes those two roles behind the existing upstream `SystemAuthUnlockProvider` boundary.
- Final Classic UX is one action: after review, `[Enter] Authenticate, sign and submit / [p] Fresnica Passphrase / [c] Cancel`. A successful platform authentication authorizes key release, signing and submission for that one CLI transaction. Cancellation fails closed. Authentication unavailability may explicitly fall back to the fresh Fresnica Passphrase.
- macOS uses `LocalAuthentication` plus the user's legacy Login Keychain. When Login Keychain is already unlocked, `LAPolicyDeviceOwnerAuthentication` provides Touch ID or macOS-password fallback; when Login Keychain itself is locked, its single unlock prompt is the authentication step. Physical Testnet acceptance proved exactly one authentication per Classic payment. Mac follow-up PR #36 was integrated into the portable base as `ad8b2fcb0b76a4a62456317ad523343e6c9d4350`.
- Windows uses Windows Hello as `DeviceAuthenticator` and Credential Manager only as `DeviceSecretStore`. Each interactive transaction calls current-user verification through `IUserConsentVerifierInterop::RequestVerificationForWindowAsync`; only `Verified` permits Credential Manager read. Cancel aborts; unavailable/not-configured/unsupported Hello explicitly requires the fresh Fresnica Passphrase. No UAC, service, LocalSystem broker or helper exists. Current desktop interop requires Windows 11 build 22000+. Product `67eae19...` makes enrollment a hard capability gate: `device-unlock enable` first performs a real desktop `IUserConsentVerifierInterop::RequestVerificationForWindowAsync` with an enrollment-specific reason. Only `Verified` proceeds to the Fresnica Passphrase and `CredWriteW`; cancelled/unavailable/not-configured Hello aborts before the Passphrase prompt and writes nothing. Transaction-time unavailable results still explicitly fall back to the fresh Fresnica Passphrase. Physical acceptance is now PASS on a Windows Hello-enabled machine: enable, disable and Testnet payment all followed the required flow.
- Linux dedicated-collection authentication was rejected after physical GNOME Keyring testing. The first build hit GNOME's custom-alias restriction; the empty-alias fix then reached the real product behavior: GNOME asked the user to create a separate password for `Fresnica Device Unlock`. That is technically valid Secret Service behavior but defeats the product goal by introducing another Fresnica-related password, so Secret Service `Lock`/`Unlock` is no longer used as the Linux authenticator.
- Linux product `acc979c...` restores the intended split: `DeviceAuthenticator` is current-user fingerprint verification through fprintd (`Claim("") -> VerifyStart("any") -> VerifyStatus` on the system D-Bus), while `DeviceSecretStore` stores the exact-envelope 32-byte key in the user's already-existing default Secret Service collection. Fresnica never creates, locks or unlocks a keyring. The default collection must already be unlocked by the desktop session; if it is locked at transaction time, Device Unlock explicitly falls back to the fresh Fresnica Passphrase without touching keyring state.
- Portable Linux Device Authentication is deliberately fingerprint-only under the one-binary/no-helper/no-install boundary. polkit would require a Fresnica action/policy installation and PAM would require a service policy while moving OS login credentials into Fresnica's conversation boundary. If fprintd, a usable reader or an enrolled fingerprint is unavailable, `enable` fails before the Fresnica Passphrase and stores nothing; transaction-time loss of fingerprint availability may explicitly fall back to the fresh Fresnica Passphrase. A true fingerprint mismatch is an authentication failure, not an availability fallback. Fresnica permits at most three completed no-match attempts; fprintd quality-retry statuses remain inside the same attempt.
- Provider cancellation remains distinct from unavailability where the platform supplies cancellation. On Linux, the user can cancel before starting via Fresnica's `[c]` path; once fprintd verification starts, mismatch fails closed and process interruption exits the one-shot CLI. The retained upstream `SystemAuth*` names are implementation terminology and remain unchanged in this slice.
- Focused VPS gate for Linux fprintd/default-keyring product `acc979c...`: repository boundary PASS; rustfmt + `git diff --check` PASS; workspace tests PASS (Anchor 7, CLI 50, CLI contract 2, presentation 3, TUI 21); workspace Clippy `-D warnings` PASS; Linux native release PASS. A targeted source audit confirms the Linux implementation contains no Secret Service collection creation, lock or unlock call. Exact-product CI/Release packaging and physical fingerprint acceptance remain open.
- Physical acceptance is PASS for macOS and Windows Hello and remains OPEN only for Linux fprintd fingerprint behavior. Tester guides are `docs/testing/windows-device-unlock-acceptance.md` and `docs/testing/linux-device-unlock-acceptance.md`.
