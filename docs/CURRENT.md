...
Token operation foundation now has a shared headless resolver at upstream `3f17593b5425fa1df136457fbe3badbd840a931a`: XLM/native and CODE:GISSUER deterministically resolve to their network-scoped SAC Contract ID, while C... remains a direct contract reference. Terminal `token TOKEN [--json]` is the first thin consumer and also accepts saved Contract Store names; a fresh-home Testnet probe showed `token XLM` and saved `native-sac` resolving to `CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC` with identical SEP-41 0.5.1 evidence.

The Contract Store commands are local operations and remain usable even when Horizon/RPC configuration is invalid. Machine `contract list --json` uses `fresnica-contract-list-v1`, separate from the on-disk Store schema. Final local validation through the token-inspect slice: rustfmt and `git diff --check` PASS; workspace Clippy `-D warnings` PASS; workspace tests PASS (Anchor 7, CLI 72, CLI integration 3, presentation 3, TUI 21); workspace release build PASS; repository boundary PASS at upstream `3f17593b...`. Upstream rust-client validation is 212/212 tests PASS. A fresh-home live Testnet probe of native XLM SAC `CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC` returned and persisted `SEP-41 0.5.1: native_sac=true, sep47_declared=false, interface_compatible=true`.
