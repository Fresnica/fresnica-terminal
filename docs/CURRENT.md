...
Status: **v0.3.0 released; one-shot System Auth is the active post-release slice. macOS is physically accepted; Linux and Windows high-trust providers are implemented and locally validated, with their real desktop user-presence acceptance still open.**
...
- Active Terminal branch: `feat/terminal-system-auth-windows`; current checkpoint `9dc54225846e9f192153c329b348613128ef0b5b`; exact upstream pin `ecdc6bf77741949e4f7043153143824ae78a1b44`.
...
- Terminal `9dc5422...` uses the same one-shot source for Payment, Trustline, SDEX, Anchor SEP-10/host payments and contract writes, adds the signed macOS companion path, the fixed-path Linux high-trust provider, and the fixed-path Windows provider + LocalSystem broker. Non-TTY invocation never activates System Auth; no CLI session/cache exists.
...
- Apple provider policy is `deviceOwnerAuthentication` / `userPresence`, allowing OS biometric retries and device-passcode fallback. The EC domain key is created and retrieved in the Data Protection Keychain and is gated by `userPresence`. The development wrapper uses a shared Xcode scheme, resolves the concrete local Mac destination ID, prints the Provisioning UDID, and requests Xcode automatic device registration/provisioning; unsigned builds never claim functional System Auth.
- Validation: upstream Required CI #144 PASS; latest Terminal local boundary, workspace tests (7 Anchor + 51 CLI + 2 CLI contract + 5 Linux provider + 5 Windows protocol/crypto + 3 presentation + 21 TUI) and workspace Clippy `-D warnings` PASS. Windows MSVC cross-check and Clippy `-D warnings` PASS for both the provider/service crate and CLI all-targets; linked x64 PE outputs exist for provider, service and CLI, and optimized provider/service PE builds PASS. No new GitHub CI or release workflow was triggered. Historical Release Terminal #92 remains the macOS shared-scheme provider compile/package evidence. Process-provider tests prove raw-key pipe transport and preservation of passphrase-required/cancel/fatal outcomes. On 2026-09-09 the development installer completed successfully on a physical Mac after the concrete-destination provisioning fix. User-executed physical acceptance proved: enrollment with fresh Fresnica Passphrase; `status` without authentication; routine Testnet signing with Touch ID; biometric failure transitioning to the macOS password fallback; successful signing with the macOS password; cancellation from the password prompt aborting with `System authentication cancelled` rather than silently downgrading to Fresnica Passphrase; and the full `disable -> status disabled -> re-enable -> sign` lifecycle, with signing restored after re-enrollment.
- macOS System Auth physical acceptance is complete for this milestone. Exact-envelope re-protection invalidation remains covered by unit tests; Terminal has no passphrase-rotation/re-protection command, so no artificial acceptance-only product surface was added.
- Linux System Auth implementation checkpoint `ed62421...` is preserved on `feat/terminal-system-auth-linux`: the CLI uses only fixed `/usr/libexec/fresnica-system-auth-provider`, never `PATH`; the separately installed root-owned/setuid provider stores UID-scoped unlock-key records under root-only `/var/lib/fresnica-system-auth` and gates every release through polkit action `com.fresnica.system-auth.release` using exact `pid,start-time,uid`. Policy is `auth_self` only for active local sessions with no `*_keep` authorization caching. `pkcheck` denial/no-agent maps to explicit Passphrase fallback, prompt dismissal maps to cancellation, and malformed provider/storage state fails closed. VPS evidence: provider unit tests PASS; root-owned `4755` install and policy load PASS; non-root probe PASS; missing enrollment maps to exit 10; the headless VPS polkit check returns not-authorized/fallback as expected. A real desktop Linux user-presence success/cancel test remains physical acceptance, not a code blocker.
- Windows System Auth implementation checkpoint `9dc5422...` is active on `feat/terminal-system-auth-windows`: the CLI loads only `%ProgramFiles%\Fresnica\SystemAuth\fresnica-system-auth-provider.exe`; a narrow `FresnicaSystemAuth` LocalSystem service owns DPAPI-protected, SID-scoped enrollment state under `%ProgramData%\Fresnica\SystemAuth`, accepts only the canonical installed provider process, and never treats DPAPI as user-authentication proof. First domain enrollment uses the Win32 WebAuthn platform authenticator with `userVerification=required` and RSA-2048/RS256; the service validates RP hash, UP/UV/attested-credential flags, exact credential id and COSE public key. Routine release uses fresh service-generated client data and the exact stored credential; the service verifies RP hash, UP/UV flags, credential id and RS256 assertion before returning the 32-byte unlock key. Removing the final signer enrollment also removes Fresnica domain metadata so `disable -> re-enable` can create a fresh platform credential after external invalidation. Local evidence: Windows protocol/crypto tests 5/5 PASS; provider/service and CLI MSVC all-target checks + Clippy PASS; debug provider/service/CLI and optimized provider/service x64 PE linking PASS. The PowerShell installer was statically reviewed on Linux but not executed because this VPS has no Windows/PowerShell environment. Real Windows installation plus Hello PIN/biometric success, cancellation, unavailable/fallback and lifecycle recovery remain physical acceptance, not claimed results.
- Physical-acceptance distribution is prepared for external testers without changing the product checkpoint: `docs/testing/linux-system-auth-acceptance.md` and `docs/testing/windows-system-auth-acceptance.md` are the handoff guides. VPS-built x86_64 archives use package source tree `40c54873ad8543c68434c029958e5f4a39d5a348`, System Auth product checkpoint `9dc54225846e9f192153c329b348613128ef0b5b`, and guide source `e231d36be3cbf7673440311581fd32124059078e`. Final archive hashes: Linux `fresnica-system-auth-linux-x86_64-40c5487-test.tar.gz` = `dcd0d65de8e02ac5d88093f43f5d8622a38c28c86e38787ce3dc1277bad2c51c`; Windows `fresnica-system-auth-windows-x86_64-40c5487-test.zip` = `53bdf4dad0e69605d968842d3c45b2bede631fd359ee18f88c7fe0425fd9c016`. Both archives were re-extracted and every internal SHA256 verified; the packaged Linux CLI ran `--version`, and all three Windows executables were verified as PE (`MZ`) outputs. The archives live outside the repository on the build VPS and are not committed binaries.
