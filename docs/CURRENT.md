...
- Active v0.5 branch: `feat/terminal-v0.5-contract-ux`. First direct-contract checkpoint: `7a761a6ec221698536bc47a3c26f8dfb190c8f87`. Current Operation Foundation product checkpoint: `bbf0f8def36c23bac5b0e195859448b8e84b7a6b`.
...
Machine discovery is now explicit at product checkpoint `bbf0f8def36c23bac5b0e195859448b8e84b7a6b`: `fresnica capabilities --json` returns versioned schema `fresnica-capabilities-v1` with CLI/source identity plus 17 currently machine-ready read/write operations and their usage/effect/dependencies. The command is deliberately zero-state: integration coverage removes HOME/USERPROFILE/FRESNICA_HOME, supplies invalid Horizon/RPC URLs, keeps a normal leading `--network` global option, and still succeeds without WalletStorage or provider initialization. Commands without a deliberate JSON contract are not advertised yet.
...
The Contract Store commands are local operations and remain usable even when Horizon/RPC configuration is invalid. Machine `contract list --json` uses `fresnica-contract-list-v1`, separate from the on-disk Store schema. Final local validation through the headless-token refactor: rustfmt and `git diff --check` PASS; workspace Clippy `-D warnings` PASS; workspace tests PASS (Anchor 7, CLI 75, CLI integration 4, presentation 3, TUI 21); workspace release build PASS; repository boundary PASS at upstream `a190244...`. Upstream rust-client validation is 220/220 tests PASS. The two former CLI numeric-format tests moved with their implementation into the shared client suite rather than being dropped. A fresh-home live Testnet probe of native XLM SAC `CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC` returned and persisted `SEP-41 0.5.1: native_sac=true, sep47_declared=false, interface_compatible=true`.
