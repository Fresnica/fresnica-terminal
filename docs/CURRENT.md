...
- Active v0.6 corpus branch: `feat/terminal-v0.6-abi-corpus`, stacked on the boundary/composer work; it pins Fresnica shared source `ad759b0cb305746e114195ea55af858c25171782` from `feat/rust-client-abi-composer-corpus`. Earlier v0.6 checkpoints are shared `ca2893b` / Terminal `0126e85` for the semantic ABI schema, shared `da21d835` / Terminal `c6b4baf` for typed JSON composition, and shared `e68c55c` / Terminal `aa18875` for UDT boundary hardening.
...
Local validation on shared `ad759b0` is 254/254 tests PASS. Terminal validation with that exact pin is rustfmt PASS, workspace tests PASS (Anchor 7, CLI 97, CLI integration 5, presentation 3, TUI 21), and workspace Clippy `-D warnings` PASS. Existing real proofs still include Blend `Vec<Request>`, smart-account Union/Enum and `Map<UDT,...>`. The corpus slice adds a live smart-account `execute(... target_args: Vec<Val>)` simulation where `{"u32":0}` reached the contract as `[0]` before failing only on Soroban's contract re-entry rule. Testnet XLM SAC `balance` and `decimals` also confirm machine responses expose exact argument `scval_xdr` and `result_xdr` alongside normalized values.

MST remains deliberately out of scope. Before exposing the Composer as an external guided-input contract, the ABI schema still needs an explicit machine-readable composition-support signal so callers do not have to infer the `Val` / `Result` / `Error` boundary themselves.
