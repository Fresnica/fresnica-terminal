...
Status: **v0.5.0 is released on Main. v0.6 is defined as Soroban ABI Composer + documentation: the Composer contract is frozen on feature branches and the remaining release work is human/AI-agent documentation and validation; Main remains unchanged pending explicit integration/release.**
...
- Active v0.6 documentation-closeout branch: `feat/terminal-v0.6-docs`, stacked on frozen Terminal Composer head `a19388b37a51f6857897d7168c3c3b057062cc9d`; it still pins Fresnica shared source `188fbbc6e17cd43419407bdc1c13371013e8e9dc` from `feat/rust-client-abi-composition-contract`. Earlier v0.6 checkpoints are shared `ca2893b` / Terminal `0126e85` for the semantic ABI schema, shared `da21d835` / Terminal `c6b4baf` for typed JSON composition, shared `e68c55c` / Terminal `aa18875` for UDT boundary hardening, shared `ad759b0` / Terminal `f76041e` for corpus/value-identity hardening, and shared `188fbbc` / Terminal `7030ace` for the machine-readable composition contract.
