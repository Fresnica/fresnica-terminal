...
Status: **v0.3.0 released; one-shot System Auth is the active post-release slice. Shared Classic/Soroban signing coordination, the macOS high-trust provider boundary, development-signed packaging and core physical user-presence acceptance are implemented.**
...
- Validation: upstream Required CI #144 PASS; latest Terminal local boundary, workspace tests (7 Anchor + 50 CLI + 2 CLI contract + 3 presentation + 21 TUI) and workspace Clippy `-D warnings` PASS. Release Terminal #92 PASS, including the macOS shared-scheme provider compile/package gate. Process-provider tests prove raw-key pipe transport and preservation of passphrase-required/cancel/fatal outcomes. On 2026-09-09 the development installer completed successfully on a physical Mac after the concrete-destination provisioning fix. User-executed physical acceptance then proved: enrollment with fresh Fresnica Passphrase; `status` without authentication; routine Testnet signing with Touch ID; biometric failure transitioning to the macOS password fallback; successful signing with the macOS password; and cancellation from the password prompt aborting with `System authentication cancelled` rather than silently downgrading to Fresnica Passphrase.
- Remaining macOS acceptance is narrow lifecycle hardening: physical `disable -> status disabled -> re-enable -> sign` evidence. Exact-envelope re-protection invalidation is already covered by unit tests; Terminal has no passphrase-rotation/re-protection command, so this milestone does not invent one solely for acceptance. Windows and Linux providers remain separate later platform slices.
