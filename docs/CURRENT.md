...
- Focused VPS gate for Linux enrollment hard-gate product `6e69287...`: repository boundary PASS; rustfmt + `git diff --check` PASS; CLI tests 49/49 + CLI contract 2/2 PASS; Linux CLI Clippy `-D warnings` and native release PASS. The preceding Windows hard-gate product `67eae19...` passed Windows x64 MSVC all-target check, Clippy `-D warnings`, release and Release Terminal #111 before physical acceptance.
