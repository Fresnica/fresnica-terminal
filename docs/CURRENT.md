...
- Active Terminal branch: `feat/terminal-system-auth`; current checkpoint `379df96ec102fd870d4d2b9252ac7d8f6590c51b`; exact upstream pin `ecdc6bf77741949e4f7043153143824ae78a1b44`.
...
- Terminal `379df96...` uses the same one-shot source for Payment, Trustline, SDEX, Anchor SEP-10/host payments and contract writes, and adds the signed macOS companion development path. Non-TTY invocation never activates System Auth; no CLI session/cache exists.
...
- Validation: upstream Required CI #144 PASS; Terminal local boundary/fmt/workspace Clippy `-D warnings`; latest local workspace tests remain 7 Anchor + 50 CLI + 2 CLI contract + 3 presentation + 21 TUI PASS. Release Terminal #92 PASS, including the macOS shared-scheme provider compile/package gate. Process-provider tests prove raw-key pipe transport and preservation of passphrase-required/cancel/fatal outcomes. The first real-Mac automatic-provisioning attempt still failed because the Apple team had no registered Mac device/profile.
- Remaining acceptance: rerun the development installer on the real Mac with the concrete destination-ID fix. If Apple still reports no registered device/profile, register the printed macOS Provisioning UDID in Certificates, Identifiers & Profiles -> Devices, rerun the installer, then complete physical user-context Touch ID/Face ID/system-password enable/release tests. Windows and Linux providers remain separate later platform slices.
