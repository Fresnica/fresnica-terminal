...
- Active Device Authentication branch: `feat/terminal-device-auth-cross-platform`; portable Device Unlock base `feat/terminal-device-unlock@ad8b2fcb0b76a4a62456317ad523343e6c9d4350`; current product checkpoint `67eae19b1600e8e1ad33f522fe4c2c855be4109d`; exact upstream pin `ecdc6bf77741949e4f7043153143824ae78a1b44`. Repository HEAD is authoritative for docs-only follow-up state.
...
- Focused VPS gate for Windows enrollment hard-gate product `67eae19...`: repository boundary PASS; rustfmt + `git diff --check` PASS; CLI tests 49/49 + CLI contract 2/2 PASS; Linux CLI Clippy `-D warnings` PASS; Windows x64 MSVC all-target check, Clippy `-D warnings` and release PASS. Exact-head CI/Release matrix remains the packaging gate before physical retest.
