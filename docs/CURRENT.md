...
- Active Device Authentication branch: `feat/terminal-device-auth-cross-platform`; portable Device Unlock base `feat/terminal-device-unlock@ad8b2fcb0b76a4a62456317ad523343e6c9d4350`; current product checkpoint `6e692871e8a4598d97147d666f16301abc973f4b`; exact upstream pin `ecdc6bf77741949e4f7043153143824ae78a1b44`. Repository HEAD is authoritative for docs-only follow-up state.
...
- Linux stores the key in a dedicated Secret Service collection aliased `fresnica-device-unlock`, not the user's default/login collection. Product `6e69287...` also makes Linux enrollment a hard capability gate: `enable` first ensures the dedicated collection exists and runs the same transaction authenticator (`silent lock -> raw Unlock -> real Secret Service Prompt -> Completed`). Only a real completed prompt proceeds to the Fresnica Passphrase and key storage. A silent immediate unlock, cancellation or unavailable prompt fails enable before any unlock key is stored. Each transaction repeats the same prompt requirement; end-of-transaction relock must remain silent.
...
- Focused VPS gate for Linux enrollment hard-gate product `6e69287...`: repository boundary PASS; rustfmt + `git diff --check` PASS; CLI tests 49/49 + CLI contract 2/2 PASS; Linux CLI Clippy `-D warnings` and native release PASS. Release Terminal #113 for exact product `6e69287...` also PASS: validate, Linux x64, macOS ARM64, Windows x64 and Windows smoke; Linux artifact digest `sha256:cd3a02091a3a87742b44a1162b189a4d53407d01428ae3ac18ab0429ed52a778`. The preceding Windows hard-gate product `67eae19...` passed Release Terminal #111 before physical acceptance.
