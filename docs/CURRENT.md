...
Token operation foundation now has a shared headless resolver at upstream `2c622814207df6e8f566c288d5fd549c4616b2c1`: XLM/native and CODE:GISSUER deterministically resolve to their network-scoped SAC Contract ID, while C... remains a direct contract reference. Terminal keeps this layer thin: `token TOKEN [--json]` inspects capabilities, `token TOKEN balance OWNER [--json]` performs a positional read through the generic contract pipeline, and `token TOKEN transfer AMOUNT TO [--wallet NAME] [-y] [--json]` binds `from` to the selected Fresnica wallet, converts decimal amounts exactly, resolves TO through the same wallet/contact/contract Address namespace, and reuses the generic contract authorization/sign/submit path. A fresh-home Testnet probe showed `token XLM` and saved `native-sac` resolving to `CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC` with identical SEP-41 0.5.1 evidence. A live positional XLM balance read returned raw `455350000001`, decimals `7`, amount `45535.0000001`; a watch-only Testnet transfer reached real simulation/preparation and then failed closed at ledger authorization with no submission output.

The Contract Store commands are local operations and remain usable even when Horizon/RPC configuration is invalid. Machine `contract list --json` uses `fresnica-contract-list-v1`, separate from the on-disk Store schema. Final local validation through the positional token-transfer slice: rustfmt and `git diff --check` PASS; workspace Clippy `-D warnings` PASS; workspace tests PASS (Anchor 7, CLI 76, CLI integration 3, presentation 3, TUI 21); workspace release build PASS; repository boundary PASS at upstream `2c622814...`. Upstream rust-client validation is 215/215 tests PASS. A fresh-home live Testnet probe of native XLM SAC `CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC` returned and persisted `SEP-41 0.5.1: native_sac=true, sep47_declared=false, interface_compatible=true`.
