...
- State is explicit: `unavailable`, `disabled`, `locked`, `ready`. macOS/Linux can report a locked vault and request the OS-native unlock prompt only when signing actually needs the key. If the vault is already unlocked, Fresnica reports `ready`; interactive signing now requires an explicit choice to use Device Unlock, switch to a fresh Fresnica Passphrase, or cancel before any unlock key is released. Windows Credential Manager has no equivalent vault-lock state in this baseline, so an existing credential is `ready`.
...
- Validation for product checkpoint `ad02cf5...`: boundary pin PASS; fmt + `git diff --check` PASS; workspace tests PASS (7 Anchor + 48 CLI + 2 CLI contract + 3 presentation + 21 TUI); workspace Clippy `-D warnings` PASS; Linux release build PASS and reports `fresnica 0.3.0 · Fresnica source ecdc6bf77741`; Windows x64 MSVC all-target check + Clippy `-D warnings` + release PE build PASS. The exact current `device_unlock_macos.rs` also passes an isolated `x86_64-apple-darwin` type-check harness with `-D warnings`; a full macOS CLI cross-build on this Linux VPS still stops earlier in the existing `ring` C build because no Apple C cross toolchain is installed. Physical macOS Testnet evidence now proves the Login Keychain `ready` state and a successful Device Unlock payment submission. That test also exposed that `ready` signed immediately after transaction confirmation; the follow-up UX patch adds an explicit Device Unlock / Passphrase / Cancel signing-method choice and passes 49 CLI tests, CLI contract 2/2, CLI Clippy `-D warnings`, and Windows x64 MSVC all-target check. The macOS `locked`/OS-prompt path remains open because the first manual `security lock-keychain` command targeted an invalid parsed path. Linux and Windows desktop/runtime Device Unlock remain unclaimed.
